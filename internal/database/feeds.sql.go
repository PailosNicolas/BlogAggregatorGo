...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, name, url, user_id, created_at, updated_at, last_updated_at
`
...
SELECT id, name, url, user_id, created_at, updated_at, last_updated_at FROM feeds
`
...
const updateLastUpdatedAt = `-- name: UpdateLastUpdatedAt :one
UPDATE feeds
...
func (q *Queries) UpdateLastUpdatedAt(ctx context.Context, id uuid.UUID) (Feed, error) {
	row := q.db.QueryRowContext(ctx, updateLastUpdatedAt, id)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.Name,
